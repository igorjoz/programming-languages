...
procedure Simulation is
   ...
   begin
      ...
      loop
         delay Duration(Random_Production.Random(G)); --  symuluj produkcj
         ...
         select
            ...
            Put_Line("Producer: Buffor not available at this moment.");
         end select;
      end loop;
   end Producer;
   ...
   begin
      ...
      loop
         ...
         select
            B.Deliver(Assembly_Type, Current_Order_Nb);

            if (Current_Order_Nb = -1) then
               Put_Line(Consumer_Name(Consumer_Nb) & ": Buffor doesn't have enough products for " & Assembly_Name(Assembly_Type));
            else
               Put_Line(Consumer_Name(Consumer_Nb) & ": ordered successfully " &
                          Assembly_Name(Assembly_Type) & " number " &
                          Integer'Image(Current_Order_Nb));
               Current_Order_Nb := Current_Order_Nb + 1;
            end if;
         else
            Put_Line(Consumer_Name(Consumer_Nb) & " Buffor is not available at this moment.");
         end select;
      end loop;
   end Consumer;
   ...
   task body Buffer is
      ...
      function removeMax return Integer is
         maxProductType: Integer := 1;
      begin
         for pType in Product_Type loop
            if (Storage(pType) > Storage(maxProductType)) then
               maxProductType := pType;
            end if;
         end loop;

         Storage(maxProductType) := Storage(maxProductType) - 1;
         In_Storage := In_Storage - 1;
         return maxProductType;
      end removeMax;
      ...
      loop
         select
            accept Take(Product: in Product_Type; Number: in Integer) do
               if not Can_Accept(Product) then
                  removed := removeMax;
                  Put_Line("Removed: " & Product_Name(removed));
               end if;

               Put_Line("Accepted  " & Product_Name(Product) & " number " &
         ...
         or
               ...
               if Can_Deliver(Assembly) then
                  Put_Line("Delivered " & Assembly_Name(Assembly) & " number " &
